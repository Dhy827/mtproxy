...
RUN set -ex \
    && apt-get update \
    && apt-get install -y --no-install-recommends git wget curl build-essential libssl-dev zlib1g-dev libzstd-dev libzstd1 iproute2 vim-common net-tools procps python3 python3-cryptography unzip \
    ...
    && rm -rf ~/go \
    && mkdir -p $WORKDIR/runtime/pid \
    && apt-get purge -y git build-essential libssl-dev zlib1g-dev libzstd-dev \
    && apt-get clean \
